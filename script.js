...
const descriptions = {
  ...
  Telnet:
    "Telnet is a protocol that allows remote access to devices and systems. Port 23 is used for Telnet connections, which provide a text-based interface to control and manage devices remotely. However, Telnet is considered insecure, and the use of SSH is recommended instead.",
  ...
  "NetBIOS/NetBT":
    "NetBIOS (Network Basic Input/Output System) and NetBT (NetBIOS over TCP/IP) are protocols used for communication between devices on a local network. Ports 137 and 139 are used for NetBIOS Name Service and NetBIOS Session Service, respectively.",
  ...
  HTTPS:
    "Hypertext Transfer Protocol Secure (HTTPS) is the secure version of HTTP, providing encrypted communication for secure web browsing. Port 443 is the default port for HTTPS connections, ensuring secure data transmission over the Internet.",
  "SMB/CIFS":
    "Server Message Block (SMB) is a file- and printer-sharing protocol commonly used by Microsoft Windows. Port 445 is used for SMB/CIFS communication, allowing devices to share files, printers, and other resources over a network.",
...
function checkAnswers() {
  ...
  serviceItems.forEach((service) => {
    ...
    const matchedPort = document.querySelector(
      `#portList li.locked[data-port="${servicePort}"]`
    );
    if (!matchedPort) {
      allCorrect = false;
    }
...
function displaySuccessMessage() {
  ...
  message.textContent =
    "Amazing! You are one step away from mastery. Don't give up.";
...
submitButton.addEventListener("click", () => {
  const allAnswersCorrect = checkAnswers(); // Check if all answers are correct

  if (allAnswersCorrect) {
    modal.style.display = "block"; // Show the modal if answers are correct
  } else {
    alert("Not all matches are correct. Please try again."); // Alert if answers are incorrect
  }
});
